...
import { CLOUD_CONFIG } from '@/constants/config';
...
export default function CameraComponent({ onCapture }: { onCapture: (uri: string) => void }) {
  ...
  useEffect(() => {
    GoogleSignin.configure({
      webClientId: '285397289693-pvvd5rbh63cbuf8ers24emtnh5a6vksa.apps.googleusercontent.com',
    });
  }, []);
  ...
  const sendPictures = async () => {
    try {
      if (!userInfo) {
        Alert.alert('Authentication Required', 'Please log in to send pictures');
        return;
      }
      ...
      if (dirInfo.exists) {
        ...
        const photoFiles = files.filter(file => 
          file.startsWith('temp_photo_') && file.endsWith('.jpg')
        );
